...
```
.
├── corpus/                      # JSON corpus files (base/contrast prompt pairs)
...
├── entropy_compute.py           # EntropyRecord dataclass, entropy computation
                                 #   (re-exports CkRecord for backward compat)
...
├── dashboard.py                 # Gradio analysis dashboard (no computation)
...
This computes the exact logit decomposition `c_k = σ_k · (r · v_k)` where v_k
and σ_k are the k-th right singular vector and singular value of W_U. Produces
nine figures per hook type:
...
post-hoc and intervention ablation, and c_k spectrum analysis across GPT-2
(small–XL) and Pythia (160M–6.9B). An interactive Gradio dashboard provides
rapid visual exploration of all precomputed results without re-running any model
inference. Active and planned directions are tracked in `FutureWork.md`, including:
